/**
 * App Runtime - Communication layer between Expo web app and parent container
 *
 * Simplified flow:
 * 1. initAppRuntime() called
 * 2. Send 'appDevServerReady' to parent to signal app is ready
 *
 * User will manually login via the app's login page.
 */
...
function sendToParent(type: MessageType, payload: Record<string, unknown> = {}): void {
  ...
  const message: PreviewerMessage = {
    type: "SpacePreviewerChannel",
...
function handleMessage(event: MessageEvent<unknown>): void {
  const data = event.data as PreviewerMessage | undefined;
 ...
 */
export function subscribeToSafeAreaInsets(callback: SafeAreaCallback): () => void {
...
/**
 * Initialize App Runtime - notifies parent that app is ready
 */
